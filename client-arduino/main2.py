...
normalizeAVGTemperature = NormalizeLastNumber(10)
withOutFilter = WithOutFilter()
normalizeDigitalFilter = NormalizerDigitalFilter(0.8, 0.5)
normalizeAVG = NormalizeLastNumber(3)
arrayFilterTemperature = [withOutFilterTemperature, normalizeDigitalFilterTemperature, normalizeAVGTemperature]
...
while True:
    ...
    if(number2 > 0.9):
        ...
        dataProcessorTemperature.sendData(temperature+0.3)
        dataProcessorPressure.sendData(pressure+0.2)
        continue
    # if(number > 0.5):
